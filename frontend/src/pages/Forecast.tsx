    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Forecast Horizon</p>
          <p className="mt-4 text-3xl font-bold text-white">{summary?.forecast_horizon ?? "7 Days"}</p>
        </Card>
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">High-Risk Clusters</p>
          <p className="mt-4 text-3xl font-bold text-white">{summary?.predicted_high_risk_hotspots ?? 3}</p>
        </Card>
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Average Forecasted EIS</p>
          <p className="mt-4 text-3xl font-bold text-white font-mono">{summary?.avg_predicted_eis?.toFixed(1) ?? "75.3"}</p>
        </Card>
      </div>

      {/* Forecast Table */}
      <Card className="rounded-[32px] border border-slate-800 bg-slate-950/45 p-6 overflow-hidden">
        <h3 className="text-md font-semibold text-white mb-4 font-display">Predicted Risk Escalation Nodes</h3>
