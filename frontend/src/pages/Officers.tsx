    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Total Officers Deployed</p>
          <p className="mt-4 text-3xl font-bold text-white">{total_officers_allocated}</p>
        </Card>
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Hotspots Covered</p>
          <p className="mt-4 text-3xl font-bold text-white">{hotspots_covered}</p>
        </Card>
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/70 p-5 shadow-lg">
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Deployment Window</p>
          <p className="mt-4 text-lg font-semibold text-cyan-300 truncate">{deployment_window}</p>
        </Card>
      </div>

      <div className="grid gap-6 xl:grid-cols-[300px_1fr]">
        {/* Resource Calculator Panel */}
        <Card className="rounded-[28px] border border-slate-800 bg-slate-950/60 p-5 space-y-6 h-fit">
          <div>
            <h3 className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Calculator</h3>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/60 text-sm text-slate-300">
                {allocations.map((alloc: any) => (
                  <tr key={alloc.hotspot_name} className="hover:bg-slate-900/20 transition-colors">
