...
export default function TemporalPage() {
  ...
  const selectedScore = data.find((item) => item.hotspot_id === selectedHotspotId) || data[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_350px]">
                ...
                {data.map((item) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 font-medium text-slate-200">
                      {item.hotspot_id.replace("hotspot-", "").toUpperCase().replace("-", " ")}
                    </td>
        ...
        {/* Explainability Breakdown side-card */}
        {selectedScore && (
          <Card className="rounded-[32px] border border-slate-800 bg-slate-950/70 p-6 space-y-6 h-fit shadow-[0_20px_50px_-30px_rgba(0,0,0,0.8)] border-cyan-900/30">
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Selected Node Explainability</p>
              <h3 className="text-xl font-bold text-white mt-1">
                {selectedScore.hotspot_id.replace("hotspot-", "").toUpperCase().replace("-", " ")}
              </h3>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-400 mb-1">
                  ...
                  <span className="font-mono text-slate-200">{selectedScore.frequency_score.toFixed(2)}</span>
                </div>
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-teal-400 rounded-full" style={{ width: `${selectedScore.frequency_score * 100}%` }} />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-400 mb-1">
                  ...
                  <span className="font-mono text-slate-200">{selectedScore.recurrence_score.toFixed(2)}</span>
                </div>
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-cyan-400 rounded-full" style={{ width: `${selectedScore.recurrence_score * 100}%` }} />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-400 mb-1">
                  ...
                  <span className="font-mono text-slate-200">{selectedScore.density_score.toFixed(2)}</span>
                </div>
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-indigo-400 rounded-full" style={{ width: `${selectedScore.density_score * 100}%` }} />
                </div>
              </div>

              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-400 mb-1">
                  <span>Temporal Volatility</span>
                  <span className="font-mono text-slate-200">{selectedScore.temporal_risk_score.toFixed(2)}</span>
                </div>
                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-400 rounded-full" style={{ width: `${selectedScore.temporal_risk_score * 100}%` }} />
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-800/80 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-slate-200 font-semibold">{selectedScore.severity_norm.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Exposure Score:</span>
                <span className="font-mono text-slate-200 font-semibold">{selectedScore.exposure_score.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Severity Multiplier:</span>
                <span className="font-mono text-rose-300 font-bold">x{selectedScore.severity_multiplier.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-md font-semibold pt-2 border-t border-slate-800/60">
                ...
                <span className="font-mono text-lg text-cyan-300 font-bold">{selectedScore.eis_score.toFixed(1)}</span>
              </div>
            </div>
          </Card>
        )}
